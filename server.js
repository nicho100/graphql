...
//minimist,dotenv, i compression, i log4js,i autocannon, i @apollo/server
...
const { serverql } = require('./graphql/server');
...
io.on('connection',async(client) => {
 ...
 serverql.start()//inicio el servidor con graphql
 .then(() => {
   app.use('/graphql', expressMiddleware(serverql));
   server.listen(config.puerto, () => console.log(`funciona`));
 });
